...
ll BFS()
{
    ...
    vector<string> gra(x);
    vector<vector<ll>> dist(x,vector<ll>(y,pINF));
    rep(i, x)
    {
        cin >> gra[i];
    }
    dist[start.first-1][start.second-1]  = 0;
    q.push(make_pair(start.first-1,start.second-1));
    while(!q.empty()){
        prll pr = q.front();
        q.pop();
        for(int i = 0;i<4;++i){
            ll nextx = pr.first + dx[i];
            ll nexty = pr.second + dy[i];
            if(nexty >= 0 && nextx >= 0 && nextx < x && nexty < y){
                if(dist[nextx][nexty] == pINF && gra[nextx][nexty] == '.'){
                    chmin(dist[nextx][nexty] ,dist[pr.first][pr.second] + 1); 
    ...
    return dist[goal.first-1][goal.second-1];
}
